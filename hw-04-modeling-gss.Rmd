...
gss16 <- gss16 %>%
  mutate(advfront = case_when(
    ...
    advfront %in% c("Disagree", "Strongly disagree", "Dont know") ~ "Not agree"
  )) %>%
  mutate(advfront = factor(advfront, levels = c("Not agree", "Agree")))
...
***The second model is slightly better because the mean value for roc_auc is about 0.005% higher which is better and the standard deviation is about 0.005% better for the second model also.***
...
gss16_pred_1 <- gss16_pred_1 %>%
  mutate(.pred_Agree_flipped = 1 - .pred_Agree)
...
```{r}
roc_1 <- roc_curve(gss16_pred_1, truth = advfront, .pred_Agree_flipped) %>% 
  mutate(model = "Model 1")

roc_2 <- roc_curve(gss16_pred_2, truth = advfront, .pred_Agree_flipped) %>% 
...
bind_rows(roc_1, roc_2) %>%
  ...
  #coord_equal() +
  labs(title = "ROC Curves: Model 1 vs Model 2", x = "1 - Specificity", y = "Sensitivity")
```

```{r}
roc_auc(gss16_pred_1, truth = advfront, .pred_Agree)
```
```{r}
roc_auc(gss16_pred_2, truth = advfront, .pred_Agree)
```
...
***1036 chose no, and 237 chose yes.***
...
***Bootstrapping can be used to estimate the proportion of Americans who have been harrassed by their superiors or co-workers at their job because it is a statistical method that can help estimate the uncertainty around a proportion.***

14. Calculate a 95% bootstrap confidence interval for the proportion of Americans who have been harassed by their superiors or co-workers at their job. Interpret this interval in the context of the data. 
...
set.seed(123)
results <- boot(data = harass_data, statistic = prop, R = 1000)

boot_ci <- boot.ci(results, type = "perc")
boot_ci
```

***We are 95% confident that the true proportion of Americans who have been harassed by a superior or co-worker is between 15.29% and 19.37%.***
